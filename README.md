...
| 产品壳层 | 桌面端采用左侧工作台，支持最近问诊、搜索记录、健康档案、记录中心与用药建议一处切换 |
| 个性化用药参考 | 基于年龄、慢病、过敏史与现用药，对 OTC / 家庭处理方向做更保守的筛选与提醒 |
| 健康空间 | 支持游客模式、本地保存、健康档案维护、历史会话查看、统一搜索与继续问诊 |
| 账号体系 | Supabase 邮箱密码登录 + Magic Link 登录，登录后可同步档案与问诊记录 |
| 附近医院 | 基于高德地图检索医院与路线，按分诊结果给出就诊方向参考 |
| 健康地图 | 支持多城市区域热力视图、热点摘要，以及“官方公开资料 / 趋势参考 / 匿名信号”分层展示 |
...
1. **药品推荐继续深化**  
   在现有 OTC/家庭处理筛选基础上，继续补药物相互作用、孕产期提示、家庭成员分层与复诊后提醒。

2. **更权威的健康热点地图**  
   继续增强按城市定位的公卫看板，引入更多地方疾控、卫健委、发热门诊公开入口与更细致的本地化说明。
