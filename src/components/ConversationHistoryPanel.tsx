    ...
    <section className="rounded-2xl border border-slate-200 bg-white/90 px-4 py-4 shadow-sm">
      <div className="flex items-start justify-between gap-3 flex-wrap">
        <div>
          <div className="flex items-center gap-2">
            ...
            <p className="text-sm font-semibold text-slate-800">历史会话</p>
          </div>
          <p className="text-xs text-slate-500 mt-1 leading-relaxed">
            现在会按对话线程保存问诊历史，点开后可以直接继续聊，不再只是摘要卡片。
          </p>
        </div>
        <button
          type="button"
      ...
      {sessions.length === 0 ? (
        <div className="mt-3 rounded-2xl border border-dashed border-slate-200 bg-slate-50 px-4 py-4 text-sm text-slate-500">
          还没有历史会话。完成一次问诊后，这里会自动按“会话”保存，方便下次继续。
        </div>
      ) : (
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    <p className="mt-2 text-xs leading-relaxed text-slate-600">{getPreviewText(session)}</p>
                    <div className="mt-2 flex items-center gap-3 text-[11px] text-slate-500">
                      ...
                      <span>{session.storage === 'supabase' ? '云端来源' : '浏览器保存'}</span>
                    </div>
                  </div>
